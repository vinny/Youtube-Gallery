...
switch ($search_id)
{
	case 'ego';
		...
		$template_html = 'video/video_search_result.html';
	break;
	...
	case 'v';
		...
		$sql_limit = ($sql_limit > $config['videos_per_page']) ? $config['videos_per_page'] : $sql_limit;
		$pagination_url = append_sid("{$phpbb_root_path}video/search.$phpEx", "search_id=v&amp;u=$author_id");

		$sql = 'SELECT v.*, ct.video_cat_title, ct.video_cat_id, u.username, u.user_colour, u.user_id
		FROM ' . VIDEO_TABLE . ' v, ' . VIDEO_CAT_TABLE . ' ct, ' . USERS_TABLE . ' u
		WHERE u.user_id = v.user_id
			AND ct.video_cat_id = v.video_cat_id
			AND u.user_id = ' . (int) $author_id . '
				ORDER BY v.video_id DESC';
		$result = $db->sql_query_limit($sql, $sql_limit, $sql_start);
		...
		// We need another query for the video count
		$sql = 'SELECT COUNT(*) as video_count FROM ' . VIDEO_TABLE . ' WHERE user_id = ' . (int) $author_id;
		$result = $db->sql_query($sql);
		$videorow['video_count'] = $db->sql_fetchfield('video_count');
		$db->sql_freeresult($result);
